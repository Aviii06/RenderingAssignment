...
private:
  ...
  Vivid::TransformComponent *sphereTransformComponent =
      new Vivid::TransformComponent();
  Vivid::TransformComponent *lightTransformComponent =
      new Vivid::TransformComponent();
  Vivid::Mesh *sphere;
...
public:
  void Setup() override {
    ...
    sphere =
        new Vivid::Mesh("./../assets/obj/newIcoSphere.obj", number_of_sphere);
  ...
  void Draw() override {
    Vector<Vivid::DirectionalLightComponent *> directionalLights =
        Vivid::ECS::GetAllComponents<Vivid::DirectionalLightComponent>();
    Vivid::Maths::Vec3 lightDiffuseColor =
        directionalLights[0]->GetDiffuseColor();
    Vivid::Maths::Vec3 lightSpecularColor =
        directionalLights[0]->GetSpecularColor();
...
//        rayPos.y += -38.375;
        for (unsigned int i = 0; i < number_of_sphere; i++) {
          ...
          float z2 = -(rayPos.x - translation.x) * (rayPos.x - translation.x) -
                     (rayPos.y - translation.y) * (rayPos.y - translation.y) +
...
Application *Vivid::CreateApplication() {
  ...
  Application *app =
      Application::GetInstance(1920, 1080, "Rendering Assignment", camera);
